...
echo -e "\n[*] Checking /etc/resolv.conf for configured DNS servers (Phishing/DNS Hijacking risk)..."
dns_servers=$(cat /etc/resolv.conf | grep -v '^#' | grep nameserver)
if [ -n "$dns_servers" ]; then
    echo "    Current DNS servers:"
    echo "$dns_servers" | sed 's/^/        /'
    echo "    [?] Verify these are your expected DNS servers. Unrecognized servers could indicate DNS hijacking."
else
    echo "    No nameservers found in /etc/resolv.conf."
fi

echo -e "\n[*] Checking system proxy variables (Potential MitM/Intercepting Proxy)..."
proxy_vars=$(env | grep -iE 'http_proxy|https_proxy|ftp_proxy|all_proxy')
if [ -n "$proxy_vars" ]; then
    echo "    [!] WARNING: System proxy variables detected:"
    echo "$proxy_vars" | sed 's/^/        /'
    echo "    [?] If you are not intentionally using a proxy or VPN, this could indicate traffic interception (MitM)."
else
    echo "    No obvious system proxy variables set."
fi
