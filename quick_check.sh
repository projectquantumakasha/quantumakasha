...
echo -e "\n[*] Checking for proxy environment variables (potential MitM/interception)..."
# Look for common proxy variables
proxy_vars=$(env | grep -iE 'http_proxy|https_proxy|all_proxy')
if [ -n "$proxy_vars" ]; then
    echo "    [!] WARNING: Proxy environment variables detected. Traffic might be intercepted:"
    echo "$proxy_vars"
else
    echo "    No standard proxy environment variables found."
fi

echo -e "\n[*] Checking current DNS configuration in /etc/resolv.conf..."
# Display configured nameservers
nameservers=$(grep '^nameserver' /etc/resolv.conf)
if [ -n "$nameservers" ]; then
    echo "    Current DNS Nameservers:"
    echo "$nameservers" | sed 's/^/        /'
    echo "    [?] Verify these are trusted DNS servers. Unknown servers could indicate DNS hijacking/phishing."
else
