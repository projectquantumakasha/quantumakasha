...
echo -e "\n[*] Checking system proxy environment variables for potential MitM interception..."
proxy_vars=$(env | grep -iE 'http_proxy|https_proxy|ftp_proxy|all_proxy')
if [ -n "$proxy_vars" ]; then
    echo "    [!] WARNING: Proxy environment variables are set! This could indicate a MitM setup if unexpected:"
    echo "$proxy_vars" | while read -r line; do echo "      $line"; done
else
    echo "    No proxy environment variables found."
fi

echo -e "\n[*] Checking /etc/resolv.conf for configured DNS servers (possible hijacking)..."
if [ -f /etc/resolv.conf ]; then
    dns_servers=$(grep '^nameserver' /etc/resolv.conf | awk '{print $2}')
    if [ -n "$dns_servers" ]; then
        echo "    Configured DNS nameservers:"
        echo "$dns_servers" | while read -r ns; do echo "      $ns"; done
    else
