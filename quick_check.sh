...
echo -e "\n[*] Checking for suspicious system proxy settings (Potential MitM)..."
proxy_vars=$(env | grep -iE 'http_proxy|https_proxy|ftp_proxy|all_proxy')
if [ -n "$proxy_vars" ]; then
    echo "    [!] System proxy variables are set. Verify these are intentional:"
    echo "$proxy_vars" | while read -r line; do echo "        $line"; done
else
    echo "    No standard system proxy variables detected."
fi

echo -e "\n[*] Checking DNS resolution settings (/etc/resolv.conf)..."
if [ -f /etc/resolv.conf ]; then
    nameservers=$(grep -E '^nameserver' /etc/resolv.conf)
    if [ -n "$nameservers" ]; then
        echo "    Current nameservers:"
        echo "$nameservers" | while read -r line; do echo "        $line"; done
        # Look for potentially suspicious public DNS if not expected, though context dependent.
        # Just listing them is usually enough for a quick check.
    else
