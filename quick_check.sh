...
echo -e "\n[*] Checking /etc/resolv.conf for DNS hijacking risks..."
if [ -f /etc/resolv.conf ]; then
    dns_servers=$(grep '^nameserver' /etc/resolv.conf | awk '{print $2}')
    echo "    Current DNS servers configured:"
    echo "$dns_servers" | while read -r line; do echo "        $line"; done
    echo "    (Ensure these are trusted servers provided by your ISP or known entities like 8.8.8.8, 1.1.1.1, etc.)"
else
    echo "    /etc/resolv.conf not found. DNS might be managed differently on this system."
fi
...
echo -e "\n[*] Checking for suspicious system proxy environment variables (potential MitM/Phishing)..."
proxies=$(env | grep -iE 'http_proxy|https_proxy|all_proxy')
if [ -n "$proxies" ]; then
    echo "    [!] WARNING: System proxy variables are set. Ensure these are intentional:"
    echo "$proxies" | while read -r line; do echo "        $line"; done
else
    echo "    No standard system proxy environment variables found."
fi
