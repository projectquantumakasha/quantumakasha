...
def main():
    ...
    telemetry_keywords = [
        "telemetry", "analytics", "tracking", "metrics", "log",
        "google-analytics", "doubleclick", "appspot", "1e100.net"
    ]
    ...
    for conn in established_conns:
        ...
        if raddr_ip != "N/A":
            try:
                # Set a short timeout to prevent hanging on slow DNS lookups
                socket.setdefaulttimeout(0.5)
                hostname = socket.gethostbyaddr(raddr_ip)[0]
        ...
        # Telemetry check
        is_telemetry = any(kw in hostname.lower() for kw in telemetry_keywords)
        if is_telemetry:
            print(f"    [!] TELEMETRY/TRACKING: Connection to known telemetry domain '{hostname}'")

        # Chrome/Chromium check
        if "chrome" in pname.lower() or "chromium" in pname.lower():
            print(f"    [*] Browser activity (Chrome/Chromium). Check {raddr_ip} for potential telemetry or user tracking.")
